...
const registerUser = asyncHandler(async (req, res) => {
  try {
    const { email, password, fullName, city, state, country } = req.body;
...
const verifyKyc = asyncHandler(async (req, res) => {
  try {
    ...
    if (!selfiePath || !documentPath)
      throw new ApiError(400, "Please upload both images");
    ...
    if (!kycResult)
      throw new ApiError(400, "KYC verification failed. Faces do not match");
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, null, "KYC verified successfully"));
  } catch (err) {
